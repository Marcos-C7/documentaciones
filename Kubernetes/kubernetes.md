...
* **Nodo**: una máquina física o virtual, cada nodo ejecuta las siguientes componentes:
    ...
    * **Kube proxy**: es una red proxy y laod balancer, se encarga de dirigir el tráfico al contenedor apropiado en base a la IP y el Puerto.
* **Pod**: es la unidad básica que puede ser agendada para desplegarse, este contiene los contenedores y se le asigna una IP única en el cluster, dentro del cluster sus contenedores tienen comunicación entre sí.
...
* **Control plane**: es un nodo cerebro en el que se ejecutan las principales componentes que administran el cluster, como son:
    ...
    * **Controller Manager** (`kube-controller-manager`): se encarga de la ejecución y administración de los controladores, los cuales se encargan de mantener el cluster en el estado deseado. Algunos de los controladores son:
        * **Controlador de replicación**: cada Pod debe tener un `ReplicaSet` que almacena cuantas replicas existen y cuantas son las deseadas, este controlador se encarga de que ambos números coincidan, si falta o muere alguno levanta nuevos y si hay de más los elimina. Esto convierte a Kubernetes en auto-reparable.
...
* `kubectl get endpoints`: ver los endpoints existentes.
* `kubectl describe endpoints <deploy-name>`: ver los endpoints de un deployment, estos van cambiando conforme los Pods se recrean.
...
* `kubectl get endpoints mi-app`: ver endpoints (IPs de los Pods).
...
* `kubectl expose deployment <deploy-name> [args] --port=<puerto>`: exponer un deployment mediante un puerto.
...
* `kubectl scale deployment mi-app --replicas=0`: detener los Pods del deployment sin eliminarlo.
