...
WiX es una herramienta moderna para generar instaladores para aplicaciones en Windows. La versión `v6` marca un cambio drástico respecto a la versión `v3`.
...
Estos instaladores sin simplemente una base de datos empaquetada en un archivo que incorpora todos los archivos, reglas, configuraciones, etc de instalación. Este archivo es procesado por el **Windows Installer Engine**.

WiX lo que hace es definir una especificación basada en archivos XML para definir todo lo relacionado a la instalación de una aplicación. WiX procesa esos archivos XML y genera un instalador `.msi`, es decir, solo genera la base de datos. Todo bajo los estándares más modernos de instalación.
...
La especificación en WiX es completamente modular y bien separada, ya que cada archivo de configuración se encarga de una parte lo cual permite matener una separación muy útil de los distintos componentes de instalación como veremos más adelante. 
...
Antes de entrar en la arquitectura de un proyecto WiX, tenemos que entencer muy bien el proceso de instalación de una aplicación:
* Regularmente durante la instalación se nos muestra una ventana que va mostrando al usuario distintas páginas de configuración para la instalación, como elegir la carpeta de instalación, elegir si correr el software al iniciar el sistema, elegir si crear un acceso directo en el escritorio, etc. Pero lo importante es que este proceso no instala nada, simplemente determina la confiuración de instalación.
* Una vez que se termina la configuración, entonces arranca el proceso de instalación, el cual se realiza con las configuraciones elegidas por el usuario.
* Entender esta separación nos ayudará a entender mejor la arquitectura de WiX.
* Esta separación es de utilidad en la práctica ya que podemos ejecutar el instalador resultante en modo silencioso, suprimiendo la UI, con un comando donde las configuraciones se dan como parámetros o usar los valores configurados por defecto. Así podemos usar el mismo instalador en modo silencioso para distribucion masiva sin tener que implementar otro aparte.

La arquitectura básica de un proyecto WiX es la siguiente:
* Un archivo `.wixproj`, normalmente llamado `<app-name>Installer.wixproj`, que contiene las especificaciones generales del proyecto, como las extensiones (algo así como librerías) que serán usadas en el proyecto, la versión de WiX que será usada, entre otras cosas, pero generalmente es muy pequeño en comparación con los otros archivos.
* Un archivo llamado normalmente `Product.wxs`, aunque el nombre puede ser cualquiera, pero su elemento raiz es `<wix ...>...</wix>`. En este archivo se describe unicamente el proceso de instalación, es decir, asumiendo que ya se tienen las configuraciones de instalación, se describe el proceso de instalación, obviamente soporta condicionales, aquí se pueden definir los valores por defecto de las configuraciones.
* Un archivo llamado normalmente `UI.wxs`, pero puede llamarse como sea, y su elemento raiz es `<<<<AQUI>>>>`. En este archivo se implementa la interfaz de usuario, que como vimos anteriormente puede ser suprimida al momento de ejecutar el instalador mediante comando. La interfaz de usuario es para dare la posibilidad al usuario de configurar la instalación.
* Archivos `.wxl`, su elemento raiz es `<WixLocalization ...>...</WixLocalization>`, y en este se implementan los textos de localización (traducción) para la UI y alertas.
...
**Nota**: la reparación automática funciona porque para los instaladores MSI, Windows guarda una copia completa en un directorio dentro de ``
...
<Directory Id="ProgramFilesFolder">
    <Directory Id="INSTALLFOLDER" Name="SysApp">
        <Component Id="MainExeComponent" Guid="PUT-GUID">
...
* Pero internamente sigue existiendo:
    ...
    * Solo que tú no los escribes manualmente.

## UI

Para la parte de la UI, WiX integra mediante la extensión `WixToolset.UI.wixext` interfaces predefinidas que podemos usar directamente, aunque podemos crear las nuestras propias.